...
<p><button type="button" id="buttonExpert" class="btn btn-info">Найти эксперта</button></p>
			  ...
			  <div class="col-sm-8" id="table"><h1 class="page-header" id="pageTitle">Эксперты:</h1>  <p id="searchResult"></p></div>
			    ...
			    <div class="col-sm-4" > <table class="table class="table table-hover">
...
$("#blockWaiting").slideDown();
	...
	$(".anonsLink").live('click', function(){
		anonsId=$(this).attr("newsId");
		//alert(anons[anonsId].title+' '+anons[anonsId].description);
  ...
  $.ajax({
  type: 'POST',
  url: '/cgi-glob/nlp/post.py',
  data: { name: anons[anonsId].title+' '+anons[anonsId].description},
  dataType: 'json',
...
success: function(res){
  jQuery.each(res, function(id, person) {
	  keywords = "";
	  jQuery.each(person.keyword, function(kwid, keyword){
	  keywords = keywords + keyword + "<br>";
	  });
	  $("#searchResult").append("<div id=\""+person.person_id+"\"><table class='table'><tbody><tr class='success'><td><img src="+person.photo+"  height='140' align='left' alt=''><p><h4 align='center'><a href="+id+" >"+person.fio+"</a><br></h4></p></td></tr></tbody></table></div><br>");
	  $("#kw").append("<tbody><tr class='success'><td><a rkw = "+person.rkw+" >"+person.rkw+"</a></h4></p></td></tr></tbody><br>")
	  $("#kw a").click(function(){
			var link=$(this).attr("rkw");
			//alert(link);
			$("#"+person.person_id+"").hide("slow");
			jQuery.each(person.keyword, function(kwid, keyword){
			keywords = keywords + keyword + "<br>";
				if (keyword == link)	<!--//тут должно быть слово которое приходит с URL>	
					$("#"+person.person_id+"").show("slow");
